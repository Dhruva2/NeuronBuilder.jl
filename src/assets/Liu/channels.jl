...
#################### NaV ###############################
mutable struct Na{T} <: IonChannel #mutable?
    gNa::T # scalar or variable
    mNa::T
    hNa::T
end

Na(x; reg = false) = Na(x, 0.0, 0.0) |> u -> (reg ? NeuronBuilder.RegIon(u, x, Symbol(:τ, get_name(u))) : u) #if regulated, R is initialized with the same value as g
ionic_current(::Na, sys::ODESystem) = sys.INa
external_params(::Na) = (:ENa, :τNa, :Ca_tgt, :τg)
...
mutable struct CaS{T} <: IonChannel
    gCaS::T
    mCaS::T
    hCaS::T
end

CaS(x; reg = false) = CaS(x, 20.0, 0.0) |> u -> (reg ?  NeuronBuilder.RegIon(u, x, Symbol(:τ, get_name(u))) : u)
...
mutable struct CaT{T} <: IonChannel
    gCaT::T
    mCaT::T
    hCaT::T
end

CaT(x; reg = false) = CaT(x, 0.0, 0.0) |> u -> (reg ?  NeuronBuilder.RegIon(u, x, Symbol(:τ, get_name(u))) : u)
...
mutable struct Ka{T} <: IonChannel
    gKa::T
    mKa::T
    hKa::T
end

Ka(x; reg = false) = Ka(x, 0.0, 0.0) |> u -> (reg ?  NeuronBuilder.RegIon(u, x, Symbol(:τ, get_name(u))) : u)
...
mutable struct KCa{T} <: IonChannel
    gKCa::T
    mKCa::T
end

KCa(x; reg = false) = KCa(x, 0.0) |> u -> (reg ?  NeuronBuilder.RegIon(u, x, Symbol(:τ, get_name(u))) : u)
...
mutable struct Kdr{T} <: IonChannel
    gKdr::T
    mKdr::T
end

Kdr(x; reg = false) = Kdr(x, 0.0) |> u -> (reg ?  NeuronBuilder.RegIon(u, x, Symbol(:τ, get_name(u))) : u)
...
mutable struct H{T} <: IonChannel
    gH::T
    mH::T
end

H(x; reg = false) = H(x, 0.0) |> u -> (reg ?  NeuronBuilder.RegIon(u, x, Symbol(:τ, get_name(u))) : u)
